...
static bool stop_after_first(Node *neighbor, uint64_t weight, void *context) {
    return record_neighbor(neighbor, weight, context) && false;
}
...
static void test_directed_add_remove_and_undirected_symmetry(void) {
    ...
    assert(adjacency_matrix_edge_count(directed) == 1U);
    ...
    assert(adjacency_matrix_edge_count(directed) == 0U);
    assert(!adjacency_matrix_has_edge(directed, a, b));
    ...
    assert(adjacency_matrix_edge_count(undirected) == 1U);
    ...
    assert(adjacency_matrix_edge_count(undirected) == 0U);
    assert(!adjacency_matrix_has_edge(undirected, a, b));
...
static void test_duplicate_neighbor_and_invalid_node_behavior(void) {
    ...
    log.count = 0U;
    assert(!adjacency_matrix_neighbors(graph, nodes[0], stop_after_first, &log));
    assert(log.count == 1U && log.visited[0] == nodes[1] && log.weights[0] == 4U);
