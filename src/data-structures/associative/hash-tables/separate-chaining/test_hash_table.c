...
static void test_creation_and_empty_table(void) {
    ...
    assert(hash_table_create(NULL, key_equals) == NULL);
    assert(hash_table_create(key_hash, NULL) == NULL);
    table = hash_table_create(key_hash, key_equals);
    assert(table != NULL);
...
static void test_insert_replace_and_remove(void) {
    HashTable *table = hash_table_create(key_hash, key_equals);
...
static void test_collisions_and_growth(void) {
    enum { ITEM_COUNT = 256 };
    HashTable *table = hash_table_create(constant_hash, key_equals);
