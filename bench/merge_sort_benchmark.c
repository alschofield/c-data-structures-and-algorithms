...
#include "benchmark.h"
// Provides the public merge sort API.
#include "../src/algorithms/sorting/comparison/merge-sort/merge_sort.h"

// Matches the quadratic sorts so cross-sort comparisons stay direct.
...
// Stores state shared by merge sort benchmarks.
struct SortContext {
...
static bool sort_operation(void *context) {
    ...
    return merge_sort(sort_context->items, ITEM_COUNT, compare_ints);
}
...
// Runs and prints one configured merge sort benchmark.
...
int main(void) {
    ...
    BenchmarkConfig config = {
        .name = "Merge sort shuffled input (one whole sort)",
    ...
    config.name = "Merge sort sorted input";
    config.setup = sorted_setup;
    if (!run_benchmark(&config)) {
        return 1;
    }

    config.name = "Merge sort reverse input";
