...
#include "benchmark.h"
// Provides the public doubly linked-list API.
#include "../src/data-structures/linear/linked/doubly-linked-list/doubly_linked_list.h"

// Uses the singly linked-list default so middle-operation numbers compare directly.
...
// Stores state shared by doubly linked-list benchmarks.
struct ListContext {
...
static bool empty_setup(void *context) {
    ...
    list_context->list = doubly_linked_list_create();
...
static bool populated_setup(void *context) {
    ...
    for (size_t index = 0U; index < ITEM_COUNT; index++) {
        if (!doubly_linked_list_push_back(list_context->list, &list_context->values[index])) {
            doubly_linked_list_destroy(list_context->list);
...
static bool push_front_operation(void *context) {
    ...
    if (!doubly_linked_list_push_front(
            list_context->list,
...
static bool push_back_operation(void *context) {
    ...
    if (!doubly_linked_list_push_back(
            list_context->list,
            &list_context->values[list_context->next_value]
        )) {
        return false;
...
static bool pop_front_operation(void *context) {
    ...
    if (!doubly_linked_list_pop_front(list_context->list, &list_context->out_item)) {
        return false;
    }
...
static bool pop_back_operation(void *context) {
    ...
    if (!doubly_linked_list_pop_back(list_context->list, &list_context->out_item)) {
        return false;
    }
...
static bool size_verify(void *context) {
    ...
    return doubly_linked_list_size(list_context->list) == list_context->count;
}
...
int main(void) {
    ...
    BenchmarkConfig config = {
        .name = "Doubly linked list push front",
    ...
    config.name = "Doubly linked list push back";
    config.operation = push_back_operation;
    if (!run_benchmark(&config)) {
        return 1;
    }

    config.name = "Doubly linked list pop front";
    ...
    config.name = "Doubly linked list pop back";
    config.operation = pop_back_operation;
    if (!run_benchmark(&config)) {
        return 1;
    }

    config.name = "Doubly linked list get middle";
    config.operation = get_middle_operation;
    if (!run_benchmark(&config)) {
        return 1;
    }

    config.name = "Doubly linked list insert middle";
    config.operation = insert_middle_operation;
    if (!run_benchmark(&config)) {
        return 1;
    }

    config.name = "Doubly linked list remove middle";
