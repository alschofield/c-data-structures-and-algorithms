...
#include "benchmark.h"
// Provides the public insertion sort API.
#include "../src/algorithms/sorting/comparison/insertion-sort/insertion_sort.h"
...
// Stores state shared by insertion sort benchmarks.
struct SortContext {
...
static bool sort_operation(void *context) {
    ...
    return insertion_sort(sort_context->items, ITEM_COUNT, compare_ints);
}
...
// Runs and prints one configured insertion sort benchmark.
...
int main(void) {
    ...
    BenchmarkConfig config = {
        .name = "Insertion sort shuffled input (one whole sort)",
    ...
    config.name = "Insertion sort sorted input (adaptive best case)";
    config.setup = sorted_setup;
    if (!run_benchmark(&config)) {
        return 1;
    }

    config.name = "Insertion sort reverse input (worst case)";
