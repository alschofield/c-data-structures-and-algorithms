...
#include "benchmark.h"
// Provides the public bubble sort API.
#include "../src/algorithms/sorting/comparison/bubble-sort/bubble_sort.h"
...
// Stores state shared by bubble sort benchmarks.
struct SortContext {
...
static bool sort_operation(void *context) {
    ...
    return bubble_sort(sort_context->items, ITEM_COUNT, compare_ints);
}
...
// Runs and prints one configured bubble sort benchmark.
...
int main(void) {
    ...
    BenchmarkConfig config = {
        .name = "Bubble sort shuffled input (one whole sort)",
    ...
    config.name = "Bubble sort sorted input (early exit)";
    config.setup = sorted_setup;
    if (!run_benchmark(&config)) {
        return 1;
    }

    config.name = "Bubble sort reverse input (worst case)";
