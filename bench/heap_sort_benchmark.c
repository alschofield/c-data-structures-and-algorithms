...
#include "benchmark.h"
// Provides the public heap-sort API.
#include "../src/algorithms/sorting/comparison/heap-sort/heap_sort.h"
...
// Stores state shared by heap-sort benchmark samples.
struct SortContext {
...
static bool sort_operation(void *context) {
    ...
    return heap_sort(sort_context->items, ITEM_COUNT, compare_ints);
}
...
// Runs and prints one configured heap-sort benchmark.
...
int main(void) {
    ...
    BenchmarkConfig config = {
        .name = "Heap sort shuffled input (one whole sort)",
    ...
    config.name = "Heap sort sorted input";
    config.setup = sorted_setup;
    if (!run_benchmark(&config)) {
        return 1;
    }

    config.name = "Heap sort reverse input";
